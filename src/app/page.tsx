...
export default function Page() {
  ...
  useEffect(() => {
    ...
    // 延迟 100ms 启动以确保 React Hydrate 完成且 DOM 渲染稳定
    const timer = setTimeout(() => {
      observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            entry.target.classList.add('scroll-visible');
            // 一次性触发显现后停止监听，避免往复触发造成干瘪的闪烁感
            observer.unobserve(entry.target);
          }
        });
      }, {
        root: null,
        threshold: 0.05, // 降低阈值以使在移动端及小屏下滑时更早、更自然地触发
  ...
  return (
    <div className="bg-[#050505] text-[#ededed] min-h-screen antialiased relative selection:bg-[#E5A93B]/30 select-none overflow-x-hidden">
      ...
      {/* 潜空间流体光晕背景 (Latent Space Ambient Glow) */}

...
export default function Home() {
  ...
  function copyWeChatID(id: string) {
    ...
    setWxCopied(true);
    setTimeout(() => setWxCopied(false), 2000);
  }
            ...
            <div id="contact" className="lg:col-span-7 flex flex-col justify-between bg-neutral-950 p-8 rounded-xl border border-white/5 font-mono">
              ...
              <div className="mt-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  <a href="https://space.bilibili.com" target="_blank" rel="noreferrer" onMouseMove={onLiquidMove} className="liquid-glass-btn py-3 px-4 rounded-xl text-center text-xs font-mono tracking-wider flex items-center justify-center gap-2"><span>BILIBILI ↗</span></a>
                  <a href="https://www.xiaohongshu.com" target="_blank" rel="noreferrer" onMouseMove={onLiquidMove} className="liquid-glass-btn py-3 px-4 rounded-xl text-center text-xs font-mono tracking-wider flex items-center justify-center gap-2"><span>小红书 ↗</span></a>
                  <a href="https://www.youtube.com" target="_blank" rel="noreferrer" onMouseMove={onLiquidMove} className="liquid-glass-btn py-3 px-4 rounded-xl text-center text-xs font-mono tracking-wider flex items-center justify-center gap-2"><span>YOUTUBE ↗</span></a>
                  <a href="https://www.instagram.com" target="_blank" rel="noreferrer" onMouseMove={onLiquidMove} className="liquid-glass-btn py-3 px-4 rounded-xl text-center text-xs font-mono tracking-wider flex items-center justify-center gap-2"><span>INSTAGRAM ↗</span></a>
                  <a href="https://x.com" target="_blank" rel="noreferrer" onMouseMove={onLiquidMove} className="liquid-glass-btn py-3 px-4 rounded-xl text-center text-xs font-mono tracking-wider flex items-center justify-center gap-2"><span>TWITTER / X ↗</span></a>
                  <button onClick={() => copyWeChatID('aigc_director_wechat')} onMouseMove={onLiquidMove} className="liquid-glass-btn py-3 px-4 rounded-xl text-center text-xs font-mono tracking-wider flex items-center justify-center gap-2"><span>{wxCopied ? '✓ WX COPIED' : 'WECHAT ↗'}</span></button>
                </div>
              </div>
              ...
              <div className="mt-12 pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <a href="mailto:collaboration@aigc.studio" onMouseMove={onLiquidMove} className="liquid-glass-btn text-white px-6 py-3 text-xs font-bold uppercase rounded-lg">发送提案</a>
              </div>
            </div>
          </div>
        </section>
